...
struct ReportBugAdminView: View {
    @StateObject private var mgr = BugManager()
    @AppStorage("isAdmin") private var isAdmin: Bool = false

    @State private var query = ""
    @State private var confirmDelete: BugReport?
    ...
    var body: some View {
        List {
            ForEach(filtered) { item in
                BugRow(item: item, isAdmin: isAdmin) {
                    confirmDelete = item
                }
                .contextMenu {            // ⬅️ 長按彈出
                    if isAdmin {
        ...
        .navigationTitle("問題回報管理")
        .searchable(text: $query, prompt: "搜尋標題 / 內容 / email / id")
        .onAppear { mgr.load() }
